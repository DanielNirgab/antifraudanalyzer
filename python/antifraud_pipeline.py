...
def create_run_dirs(output_dir: Path) -> dict:
    ...
    # короткий индекс, чтобы не перезаписать предыдущий запуск.
    if run_dir.exists():
        i = 2
...
def run_pipeline(input_path: str, output_path: str) -> None:
    ...
    # Оно показывает общее количество операций, но из-за дисбаланса классов мошеннические операции могут быть визуально почти незаметны.
    plt.figure(figsize=(8, 5))
